// Google Analytics is optional and remains unloaded until the visitor accepts.
// Declining stores that choice without creating analytics cookies. Cookie
// Settings can revoke later, which removes the loader and expires GA cookies.
...
export function stopAnalytics(documentImpl) {
  if (typeof window !== "undefined") {
    // Google documents this property as the immediate opt-out switch for a
    // measurement ID. Removing the loader alone would not stop code that has
    // already executed in the page.
...
export function clearGaCookies(documentImpl) {
  ...
  const apex = host === "www.tierone.bio" ? "tierone.bio" : "";
...
function persistConsent(value, storage) {
  ...
  try {
    ...
    // Some privacy modes refuse localStorage. Analytics still follows the
    // in-memory choice for this visit; the banner may reappear next time.
  }
}
