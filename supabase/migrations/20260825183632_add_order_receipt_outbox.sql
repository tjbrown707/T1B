...
-- Durable checkout receipts. The order row is already server-priced and
-- inventory-reserved before it reaches this queue, so the enqueue RPC copies
-- its immutable receipt fields directly from public.orders. One row per order
-- plus a versioned Resend idempotency key prevents duplicate customer mail
-- across browser retries, function retries, and the scheduled queue drain.
...
revoke execute on function public.enqueue_order_receipt(uuid) from public, anon, authenticated;
...
grant execute on function public.enqueue_order_receipt(uuid) to service_role;
